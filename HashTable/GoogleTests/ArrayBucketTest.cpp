#include "pch.h"
#include "../atrunov/Task1/ArrayBucket.h"
#include <string>

class ArrayBucketTest :public ::testing::Test
{
protected:
	void SetUp() override
	{
	...
	ArrayBucket<int, int> b0;
	ArrayBucket<std::string, int> b1;
	Pair<const std::string, int> B1[3] = { {"Two",2},{"Five",5},{"Three",3} };
	ArrayBucket<int, std::string> b2;
	Pair<const int, std::string> B2[4] = { { 1,"One" } ,{ 2,"Two" } ,{ 6,"Six" }, { 9,"Nine" } };
	ArrayBucket<int, bool> b3;
	Pair<const int, bool> B3[3] = { { 1,true } ,{ 2,true },{ 3,true } };
};

TEST_F(ArrayBucketTest, IsEmptyInitially)
{
	EXPECT_EQ(b0.Begin(), nullptr);
}

TEST_F(ArrayBucketTest, PushBackWorks)
{
...
TEST_F(ArrayBucketTest, SearchWorks)
{
	for (int i = 0; i < 3; i++)
	{
...
TEST_F(ArrayBucketTest, IteratorWorks)
{
	auto iter1 = b1.Begin();
	(*iter1).item = 4;
	iter1 = b1.Begin();
...
TEST_F(ArrayBucketTest, ReallocationWorks)
{
	int i;
	for (i = 0; i < 15; i++)
		b0.Push({ i,i });
	std::cout << "Test 4:"; b0.Print(); std::cout << '\n';
	i = 0;
	for (auto iter = b0.Begin(); iter != nullptr; iter++,i++)
	{
		EXPECT_EQ((*iter).key, i) << "Key value: Expected " << i << "|Received " << (*iter).key;
		EXPECT_EQ((*iter).item, i) << "Item value: Expected " << i << "|Received " << (*iter).item;
	}
}

TEST_F(ArrayBucketTest, CopyConstructorWorks)
{
	auto b5 = b2;
	auto iter2 = b2.Begin();
...
TEST_F(ArrayBucketTest, MoveConstructorWorks)
{
	auto b5 = std::move(b2);
	int i = 0;
	for (auto iter5 = b5.Begin(); iter5 != nullptr; iter5++)
...
TEST_F(ArrayBucketTest, CopyAssignmentWorks)
{
	ArrayBucket<int, std::string> b5;
	b5.Push({ 0,"Zero" });
	b5.Push({ 3,"Three" });
	b5 = b2;
	auto iter2 = b2.Begin();
...
TEST_F(ArrayBucketTest, MoveAssignmentWorks)
{
	ArrayBucket<int, std::string> b5;
	b5.Push({ 0,"Zero" });
	b5.Push({ 3,"Three" });
